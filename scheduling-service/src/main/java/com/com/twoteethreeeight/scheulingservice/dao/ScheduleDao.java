...
public interface ScheduleDao extends MongoRepository<Schedule, String> {
    ...
    @Aggregation({"{ $lookup: {from : \"airport\", localField: \"from\", foreignField: \"_id\", as: \"departure\"}}",
            ...
            "{ $match: { 'departure.0.name': { $regex: ?0, $options: 'i' }, 'destination.0.name': { $regex: ?1, $options: 'i' }, 'takeOffTime': { $gte: ?2 } } }"})
    List<Schedule> searchSchedule(String from, String to, LocalDateTime startTime);
